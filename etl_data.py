...
spark = SparkSession.builder.config("spark.jars.packages","com.datastax.spark:spark-cassandra-connector_2.12:3.5.1," "com.mysql:mysql-connector-j:8.4.0").getOrCreate()
...
def calculating_conversion(df):    
    ...
    conversion_output = spark.sql(""" select job_id, date(ts) as Date, hour(ts) as Hour, publisher_id, campaign_id, group_id, count(*) as conversion
            from conversion
            group by job_id, date(ts), hour(ts), publisher_id, campaign_id, group_id """)
...
def retrieve_company_data(url,driver,user,password):
    sql = """(SELECT id as job_id, company_id, group_id, campaign_id FROM job) AS test"""
...
def main_task(mysql_time):
    ...
    print('-----------------------------')
    df = spark.read.format("org.apache.spark.sql.cassandra").options(table="tracking",keyspace="demo").load().where(col('ts')>= mysql_time)
    print('-----------------------------')
...
def get_latest_time_cassandra():
    data = spark.read.format("org.apache.spark.sql.cassandra").options(table = 'tracking',keyspace = 'demo').load()
